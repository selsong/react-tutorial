...
# =========================== [ADD YOUR TESTS HERE] ===========================
    ...
    def test_adjacent_moves(self):
        '''Check that moving to non-adjacent square is invalid'''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        tiles[2].click()
        tiles[5].click()
        tiles[8].click()
        ...
        tiles[4].click()
        tiles[7].click()
        tiles[8].click() #try to move X from 8 to 3, invalid bc not adj
        tiles[3].click()
        self.assertTileIs(tiles[8], self.SYMBOL_X)
        self.assertTileIs(tiles[3], self.SYMBOL_BLANK)

    def test_center(self):
        '''Check that non-winning moves are invalid, must vacate center'''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        tiles[2].click()
        tiles[5].click()
        tiles[8].click()
        ...
        tiles[4].click()
        tiles[6].click()
        #must move 4 so 2 and 8 should be invalid
        tiles[2].click()
        tiles[1].click()
    ...
    def test_prevent_moves_after_win(self):
        '''Test that no moves can be made after a player wins the game.'''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)

        # Setup a winning scenario for 'X'
        tiles[0].click()  # X moves
        tiles[3].click()  # O moves
        tiles[1].click()  # X moves
        tiles[4].click()  # O moves
        tiles[2].click()  # X moves - this should be a win (top row)

        # Attempt to make another move after the win
        tiles[5].click()  # O tries to move

        # Validate that the winning state is unchanged and no new moves are recorded
        self.assertTileIs(tiles[0], self.SYMBOL_X)
        self.assertTileIs(tiles[1], self.SYMBOL_X)
        self.assertTileIs(tiles[2], self.SYMBOL_X)
        self.assertTileIs(tiles[5], self.SYMBOL_BLANK)  # Ensure this tile remains unchanged

        # Check the game status to confirm the win
        status_element = self.driver.find_element(By.CSS_SELECTOR, '.status')
        self.assertIn('Winner: X', status_element.text)  # Example status check, adjust based on actual implementation
